...
def retrieveEntries(mp, ds, search, decryptPassword = False):
	...
	query = ""
	if len(search)==0:
		query = "SELECT * FROM pmdatabase.entries"
	else:
		query = "SELECT * FROM pmdatabase.entries WHERE "
		for i in search:
	...
	if len(results) == 0:
		printc("[red][-] No results for the search[/red] ")
		return

	if (decryptPassword and len(results)>1) or (not decryptPassword):
		if decryptPassword:
			printc("[yellow][-] More than one result found for the search, therefore not extracting the password. Be more specific. [/yellow]")
